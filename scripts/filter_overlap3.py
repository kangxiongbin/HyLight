...
def main():
    ...
    parser.add_argument('-len', dest = 'min_ovlp_len', required=False, default = 1000, type = int, help='the Shortest length of overlap')
    parser.add_argument('-thre', dest = 'threshold', required=False, default = 0.0025, type = int, help='allow the number of mutations in the overlap region between two reads')
...
#    execute("sort -T /tmp -nk7 -k8 -k9  -k5 %s > %s;" %(overlap1_2, overlap_sort))

    paf_file = os.popen("cat %s" %overlap_sort)
    (snp, map_po, start_po) = prpare_mutation2(paf_file, min_ovlp_len)
...
def prpare_mutation2(paf_file, min_ovlp_len):
...
#            score = format((0.4*(int(length)/mlen)+0.6*(int(matchcount)/int(length))), '.4f')
            score = format((0.4*(length/mlen)+0.6*(1 - mis)), '.4f')

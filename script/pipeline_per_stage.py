...
def main():
    ...
    parser.add_argument('--len_c', dest='len_c', type=int, default=50000000)
    ...
    if args.stage == 'b':
        ...
        # Stage b
        run_first_it_merge(args.fastq, args.overlaps, args.edge_threshold, args.min_overlap_perc, min_overlap_len, args.merge_contigs, first_it, len_c = args.len_c)
        while overlap_counts[-1] > 0 and edge_counts[-1] > 0 and const_read_its < 2:
            while overlap_counts[-1] > 0 and edge_counts[-1] > 0 and const_read_its < 2:
                # merge simple paths
                run_merging_it(args.edge_threshold, args.min_overlap_perc, min_overlap_len, 0, len_c = args.len_c)
            ...
            if args.remove_branches == 'false':
                run_clique_it(args.edge_threshold, args.min_overlap_perc, min_overlap_len, 0, len_c = args.len_c)
                if read_counts[-1] == read_counts[-2]:
                    const_read_its += 1
                else:
                    const_read_its = 0
...
def run_first_it_merge(fastq, overlaps, edge_threshold, min_overlap_perc, min_overlap_len, error_rate, first_it, remove_branches='true', len_c = 50000000):
    ...
    copy_log()
    [singles_count, paired_count, n_overlaps] = analyze_results(cliques=False, len_c = len_c)
...
def run_merging_it(edge_threshold, min_overlap_perc, min_overlap_len, error_rate, remove_branches='true', len_c = 50000000):
    global iteration, max_read_lengths, max_coverages, overlap_counts, edge_counts
    global read_counts, singles_counts, paired_counts
    iteration += 1
    if verbose == 'true':
...
def run_clique_it(edge_threshold, min_overlap_perc, min_overlap_len, error_rate, len_c = 50000000):
    global iteration, max_read_lengths, max_coverages, overlap_counts, edge_counts
    global read_counts, singles_counts, paired_counts
    iteration += 1
    if verbose == 'true':
...
def analyze_results(cliques=False, len_c=50000000):
#    max_cov = analyze_coverage()
#    max_coverages.append(max_cov)
    [singles_count, max_len] = analyze_fastq("singles.fastq", len_c = len_c)
    if os.path.isfile('paired1.fastq'):
        [paired_count, paired_len] = analyze_fastq("paired1.fastq", len_c = len_c)
    else:
        paired_count = 0
...
def analyze_fastq(infile, len_c):
    len_counts = [0 for i in range(len_c)]
    max_len = 0

...
def main():
    ...
    | python %s/filter_trans_ovlp_inline.py -len 40 -iden %s " %(con, con, bin, iden))

    overlap_reads_contigs = os.popen("minimap2 -t 30 -cx ava-pb -Hk19 -Xw5 -m100 -g10000 --max-chain-skip 25 %s %s 2> /dev/null \
    | python %s/filter_trans_ovlp_inline.py -len 40 -iden %s " %(con, reads, bin, iden))
    ...
    # generate overlap between reads and contigs and confirm the direction.

    ovlreadf = {} # the forward nodes of a reads
    ovlreada = {} # the number of afterward nodes

    for line1 in overlap_reads_contigs:
        line = line1.split( )
#        print(line)
        ...
        if int(line[2]) < 1:
            ...
            if line[5] in ovlconf:
                ...
                if line[0] in ovlreadf:
                    ovlreadf[line[0]].append(line[5])
                    
                else:
                    ovlreadf[line[0]] = [line[5]]

        elif(int(line[7]) < 1):

            if line[0] in ovlreada:
                
                ovlreada[line[0]].append(line[5])
            else:
                ovlreada[line[0]] = [line[5]]

    # count overlap number between each two contigs
    for k in ovlreada.keys():
        if k in ovlreadf:
            for v1 in ovlreadf[k]:
                for v2 in ovlreada[k]:
                    if v1 in ovlconf:
                        keys = ":".join([v1, v2])
                        if keys in ovlcon:
                            ovlcon[keys] += 1
                            
    ovlcon_sort = sorted(ovlcon.items(), key=lambda x: x[1], reverse=True)
 
    # pick u final contigs that can be extended (you can change the threshold)
    final_nod = {}
    for key in ovlcon_sort:
        k = key[0].split(":")[0]
    
        if novlconf[k] == 1:
            final_nod[key[0]] = key[1]
            ...
            for sk in ovlconf[k]:
                ...
                if sk in ovlcon:
                    nover = nover + ovlcon[sk]
                                    
            if (ovlcon[key[0]]/nover) > perc:
    ...
    with open (args.out, "w") as out:
        for line in sfoverlaps:
            ...
            if (k1 in final_nod) or (k2 in final_nod):
                out.write(line)
